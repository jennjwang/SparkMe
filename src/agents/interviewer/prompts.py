...
✅ **For the Typical Week Description subtopic: once the participant has named tasks with clear action+object, the subtopic is covered. Do NOT drill into any individual task with incident-level recall (e.g., "the last grant you submitted", "the most recent meeting you had") or how-they-do-it follow-ups.**
...
✅ **Task recording rules (apply whenever the participant mentions a task):**
   ...
   - **The object MUST be concrete enough to identify what's being acted on.** Bare abstract nouns (e.g., "analysis", "meetings", "emails", "documents", "work", "projects", "research") are NOT specific enough on their own. When the participant names a task with a vague object, ask ONE targeted clarifying question to pin down the concrete object before recording the task. Examples:
     ...
     Stop after ONE clarification. If the participant's follow-up answer is still vague ("just general stuff", "it varies"), accept their answer, record with the best concrete framing they gave, and move on. Do NOT demand further refinement.
   - The action should likewise be concrete. Rewrite vague verbs the participant uses ("do", "handle", "work on", "deal with") to the specific action implied by the object when recording the task — do NOT record "do executive teachings"; record "teach executive education sessions".
